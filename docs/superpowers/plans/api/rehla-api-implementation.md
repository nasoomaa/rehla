...
**Spec:** `docs/superpowers/specs/api/rehla-api-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/api/rehla-api-design.md` completely.
...
- `packages/Rehla/Api/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/Api/src/Providers/ApiServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/Api/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla API V1 package boundary

**Files:**
- Create: `packages/Rehla/Api/composer.json`
- Create: `packages/Rehla/Api/src/Providers/ApiServiceProvider.php`
- Create: `packages/Rehla/Api/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `customers`, `catalog`, `checkout`, `sales`, `payment`, `applications`, `media`
- Produces: only interfaces declared in `docs/superpowers/specs/api/rehla-api-design.md` for this task's responsibility.

- [ ] **Step 1: Write the failing test or executable gate first**

  Required behavior:
  - Package autoloads and boots only approved dependencies: `core`, `customers`, `catalog`, `checkout`, `sales`, `payment`, `applications`, `media`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): bootstrap rehla api v1 package boundary"
```
...
**Files:**
- Create: `packages/Rehla/Api/src/Contracts/ApiErrorResponder.php`
- Create: `packages/Rehla/Api/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): define public contracts and boundary tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 3: Implement the smallest vertical service path

**Files:**
- Create: `packages/Rehla/Api/src/Http/Middleware/AttachRequestId.php`
- Create: `packages/Rehla/Api/src/Http/Middleware/ApiRateLimit.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/AuthController.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/CatalogController.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/CheckoutController.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/OrdersController.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/PaymentsController.php`
- Create: `packages/Rehla/Api/src/Http/Controllers/V1/ApplicationsController.php`
- Create: `packages/Rehla/Api/src/Http/Requests/V1/CheckoutRequest.php`
- Create: `packages/Rehla/Api/src/Http/Resources/V1/ServiceResource.php`
- Create: `packages/Rehla/Api/src/Http/Resources/V1/OrderResource.php`
- Create: `packages/Rehla/Api/src/Http/Resources/V1/PaymentResource.php`
- Create: `packages/Rehla/Api/src/Http/Resources/V1/ApplicationResource.php`
- Create: `packages/Rehla/Api/src/Routes/api-v1.php`
- Create: `packages/Rehla/Api/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Provide the versioned Flutter/mobile presentation layer using Sanctum, Form Requests, API Resources, consistent errors, pagination, rate limits and idempotency.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): implement the smallest vertical service path"
```

Do not include unrelated working-tree changes in the commit.

### Task 4: Implement security invariants as regression tests

**Files:**
- Create: `packages/Rehla/Api/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): implement security invariants as regression tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 5: Implement V1 HTTP presentation slice
...
- Create: `packages/Rehla/Api/src/Http/Resources/V1/ApplicationResource.php`
- Create: `packages/Rehla/Api/src/Routes/api-v1.php`
- Create: `packages/Rehla/Api/tests/Feature/ApiV1ContractTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  - Responses use Resources/error envelope and never raw models
  - Sanctum/rate limit/authorization middleware attached

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests/Feature/ApiV1ContractTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): implement v1 http presentation slice"
```

Do not include unrelated working-tree changes in the commit.

### Task 6: Run Rehla API V1 completion gate

**Files:**
- Create: `docs/superpowers/evidence/api-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Api/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-api): run rehla api v1 completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/api/rehla-api-design.md` and map every requirement to an implemented task/test.
