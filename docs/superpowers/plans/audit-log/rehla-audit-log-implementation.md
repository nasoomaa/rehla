# Rehla AuditLog Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Record security-relevant administrator/domain changes with actor, entity, request metadata and redacted before/after values.

**Architecture:** This plan implements one Rehla execution unit only. It consumes already-approved dependencies (core, admin-users), keeps the package boundary from its design spec, and defers downstream behavior to later plans. Every behavior-bearing task uses TDD and every task ends with fresh verification and a focused commit.
...
**Spec:** `docs/superpowers/specs/audit-log/rehla-audit-log-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/audit-log/rehla-audit-log-design.md` completely.
...
- `packages/Rehla/AuditLog/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/AuditLog/src/Providers/AuditLogServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/AuditLog/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla AuditLog package boundary

**Files:**
- Create: `packages/Rehla/AuditLog/composer.json`
- Create: `packages/Rehla/AuditLog/src/Providers/AuditLogServiceProvider.php`
- Create: `packages/Rehla/AuditLog/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `admin-users`
- Produces: only interfaces declared in `docs/superpowers/specs/audit-log/rehla-audit-log-design.md` for this task's responsibility.

- [ ] **Step 1: Write the failing test or executable gate first**

  Required behavior:
  - Package autoloads and boots only approved dependencies: `core`, `admin-users`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): bootstrap rehla auditlog package boundary"
```
...
**Files:**
- Create: `packages/Rehla/AuditLog/src/Contracts/AuditRecorder.php`
- Create: `packages/Rehla/AuditLog/src/Contracts/AuditQueryService.php`
- Create: `packages/Rehla/AuditLog/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/AuditLog/src/Database/Migrations/2026_08_22_160001_create_audit_log_table.php`
- Create: `packages/Rehla/AuditLog/src/Models/AuditEntry.php`
- Create: `packages/Rehla/AuditLog/tests/Feature/PersistenceTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Feature/PersistenceTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): implement package-owned persistence"
```
...
**Files:**
- Create: `packages/Rehla/AuditLog/src/Services/DatabaseAuditRecorder.php`
- Create: `packages/Rehla/AuditLog/src/Services/AuditRedactor.php`
- Create: `packages/Rehla/AuditLog/src/Services/DatabaseAuditQueryService.php`
- Create: `packages/Rehla/AuditLog/src/Support/AuditActorResolver.php`
- Create: `packages/Rehla/AuditLog/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Record security-relevant administrator/domain changes with actor, entity, request metadata and redacted before/after values.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/AuditLog/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): implement security invariants as regression tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 6: Implement redacted audit recorder

**Files:**
- Create: `packages/Rehla/AuditLog/src/Services/AuditRecorder.php`
- Create: `packages/Rehla/AuditLog/src/Services/AuditRedactor.php`
- Create: `packages/Rehla/AuditLog/tests/Feature/AuditRedactionTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Passwords/tokens/secrets/sensitive application values absent from stored before/after

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests/Feature/AuditRedactionTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): implement redacted audit recorder"
```

Do not include unrelated working-tree changes in the commit.

### Task 7: Run Rehla AuditLog completion gate

**Files:**
- Create: `docs/superpowers/evidence/audit-log-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/AuditLog/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-audit-log): run rehla auditlog completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/audit-log/rehla-audit-log-design.md` and map every requirement to an implemented task/test.
