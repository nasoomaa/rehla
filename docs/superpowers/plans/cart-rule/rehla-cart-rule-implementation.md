# Rehla CartRule Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Own promotions and coupons using the neutral Rule engine and a CartRuleContext that does not depend on Checkout models.

**Architecture:** This plan implements one Rehla execution unit only. It consumes already-approved dependencies (core, rule, catalog, customers), keeps the package boundary from its design spec, and defers downstream behavior to later plans. Every behavior-bearing task uses TDD and every task ends with fresh verification and a focused commit.
...
**Spec:** `docs/superpowers/specs/cart-rule/rehla-cart-rule-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/cart-rule/rehla-cart-rule-design.md` completely.
...
- `packages/Rehla/CartRule/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/CartRule/src/Providers/CartRuleServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/CartRule/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla CartRule package boundary

**Files:**
- Create: `packages/Rehla/CartRule/composer.json`
- Create: `packages/Rehla/CartRule/src/Providers/CartRuleServiceProvider.php`
- Create: `packages/Rehla/CartRule/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `rule`, `catalog`, `customers`
- Produces: only interfaces declared in `docs/superpowers/specs/cart-rule/rehla-cart-rule-design.md` for this task's responsibility.

- [ ] **Step 1: Write the failing test or executable gate first**

  Required behavior:
  - Package autoloads and boots only approved dependencies: `core`, `rule`, `catalog`, `customers`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): bootstrap rehla cartrule package boundary"
```
...
**Files:**
- Create: `packages/Rehla/CartRule/src/Contracts/CartRuleEvaluator.php`
- Create: `packages/Rehla/CartRule/src/Contracts/CouponRedemption.php`
- Create: `packages/Rehla/CartRule/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/CartRule/src/Database/Migrations/2026_08_22_100001_create_cart_rules_table.php`
- Create: `packages/Rehla/CartRule/src/Database/Migrations/2026_08_22_100002_create_coupons_table.php`
- Create: `packages/Rehla/CartRule/src/Database/Migrations/2026_08_22_100003_create_cart_rule_conditions_table.php`
- Create: `packages/Rehla/CartRule/src/Database/Migrations/2026_08_22_100004_create_cart_rule_actions_table.php`
- Create: `packages/Rehla/CartRule/src/Database/Migrations/2026_08_22_100005_create_cart_rule_usage_table.php`
- Create: `packages/Rehla/CartRule/src/Models/CartRule.php`
- Create: `packages/Rehla/CartRule/src/Models/Coupon.php`
- Create: `packages/Rehla/CartRule/src/Models/CartRuleCondition.php`
- Create: `packages/Rehla/CartRule/src/Models/CartRuleAction.php`
- Create: `packages/Rehla/CartRule/src/Models/CartRuleUsage.php`
- Create: `packages/Rehla/CartRule/tests/Feature/PersistenceTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/PersistenceTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): implement package-owned persistence"
```
...
**Files:**
- Create: `packages/Rehla/CartRule/src/Context/CartRuleContext.php`
- Create: `packages/Rehla/CartRule/src/Results/DiscountResult.php`
- Create: `packages/Rehla/CartRule/src/Services/DefaultCartRuleEvaluator.php`
- Create: `packages/Rehla/CartRule/src/Services/CouponRedemptionService.php`
- Create: `packages/Rehla/CartRule/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Own promotions and coupons using the neutral Rule engine and a CartRuleContext that does not depend on Checkout models.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/CartRule/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): implement security invariants as regression tests"
```
...
**Files:**
- Create: `packages/Rehla/CartRule/src/Events/CouponRedeemed.php`
- Create: `packages/Rehla/CartRule/src/Events/CartRuleApplied.php`
- Create: `packages/Rehla/CartRule/tests/Feature/EventsTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/EventsTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): implement typed events and secondary-reaction boundary"
```

Do not include unrelated working-tree changes in the commit.

### Task 7: Implement concurrency-safe redemption

**Files:**
- Create: `packages/Rehla/CartRule/src/Services/CouponRedemptionService.php`
- Create: `packages/Rehla/CartRule/tests/Feature/CouponConcurrencyTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Final available redemption cannot be consumed twice under concurrent transactions

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests/Feature/CouponConcurrencyTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): implement concurrency-safe redemption"
```

Do not include unrelated working-tree changes in the commit.

### Task 8: Run Rehla CartRule completion gate

**Files:**
- Create: `docs/superpowers/evidence/cart-rule-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/CartRule/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-cart-rule): run rehla cartrule completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/cart-rule/rehla-cart-rule-design.md` and map every requirement to an implemented task/test.
