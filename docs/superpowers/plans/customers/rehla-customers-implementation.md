...
**Spec:** `docs/superpowers/specs/customers/rehla-customers-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/customers/rehla-customers-design.md` completely.
...
- `packages/Rehla/Customers/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/Customers/src/Providers/CustomersServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/Customers/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla Customers package boundary

**Files:**
- Create: `packages/Rehla/Customers/composer.json`
- Create: `packages/Rehla/Customers/src/Providers/CustomersServiceProvider.php`
- Create: `packages/Rehla/Customers/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `media`
- Produces: only interfaces declared in `docs/superpowers/specs/customers/rehla-customers-design.md` for this task's responsibility.

- [ ] **Step 1: Write the failing test or executable gate first**

  Required behavior:
  - Package autoloads and boots only approved dependencies: `core`, `media`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): bootstrap rehla customers package boundary"
```
...
**Files:**
- Create: `packages/Rehla/Customers/src/Contracts/CustomerRepository.php`
- Create: `packages/Rehla/Customers/src/Contracts/CustomerIdentity.php`
- Create: `packages/Rehla/Customers/src/Contracts/CustomerProfileService.php`
- Create: `packages/Rehla/Customers/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/Customers/src/Database/Migrations/2026_08_22_080001_create_customers_table.php`
- Create: `packages/Rehla/Customers/src/Models/Customer.php`
- Create: `packages/Rehla/Customers/tests/Feature/PersistenceTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Feature/PersistenceTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): implement package-owned persistence"
```
...
**Files:**
- Create: `packages/Rehla/Customers/src/Repositories/EloquentCustomerRepository.php`
- Create: `packages/Rehla/Customers/src/Services/CustomerProfileManager.php`
- Create: `packages/Rehla/Customers/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Own customer identity/profile data and customer-facing authentication domain support used by Checkout and mobile API.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/Customers/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): implement security invariants as regression tests"
```
...
**Files:**
- Create: `packages/Rehla/Customers/src/Events/CustomerRegistered.php`
- Create: `packages/Rehla/Customers/src/Events/CustomerProfileUpdated.php`
- Create: `packages/Rehla/Customers/src/Events/CustomerStatusChanged.php`
- Create: `packages/Rehla/Customers/tests/Feature/EventsTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests/Feature/EventsTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): implement typed events and secondary-reaction boundary"
```

Do not include unrelated working-tree changes in the commit.

### Task 7: Run Rehla Customers completion gate

**Files:**
- Create: `docs/superpowers/evidence/customers-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Customers/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-customers): run rehla customers completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/customers/rehla-customers-design.md` and map every requirement to an implemented task/test.
