# Rehla DataGrid Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Provide a safe reusable server-side DataGrid engine with registered columns, filters, sorting, pagination, actions and export contracts.
...
**Spec:** `docs/superpowers/specs/datagrid/rehla-datagrid-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/datagrid/rehla-datagrid-design.md` completely.
...
- `packages/Rehla/DataGrid/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/DataGrid/src/Providers/DataGridServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/DataGrid/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla DataGrid package boundary

**Files:**
- Create: `packages/Rehla/DataGrid/composer.json`
- Create: `packages/Rehla/DataGrid/src/Providers/DataGridServiceProvider.php`
- Create: `packages/Rehla/DataGrid/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`
- Produces: only interfaces declared in `docs/superpowers/specs/datagrid/rehla-datagrid-design.md` for this task's responsibility.
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): bootstrap rehla datagrid package boundary"
```
...
**Files:**
- Create: `packages/Rehla/DataGrid/src/Contracts/DataGridContract.php`
- Create: `packages/Rehla/DataGrid/src/Contracts/FilterContract.php`
- Create: `packages/Rehla/DataGrid/src/Contracts/ExporterContract.php`
- Create: `packages/Rehla/DataGrid/src/Contracts/ActionAuthorizer.php`
- Create: `packages/Rehla/DataGrid/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/DataGrid/src/GridRegistry.php`
- Create: `packages/Rehla/DataGrid/src/DataGrid.php`
- Create: `packages/Rehla/DataGrid/src/Columns/Column.php`
- Create: `packages/Rehla/DataGrid/src/Filters/Filter.php`
- Create: `packages/Rehla/DataGrid/src/Query/GridQuery.php`
- Create: `packages/Rehla/DataGrid/src/Query/GridQueryProcessor.php`
- Create: `packages/Rehla/DataGrid/src/Results/GridResult.php`
- Create: `packages/Rehla/DataGrid/src/Actions/RowAction.php`
- Create: `packages/Rehla/DataGrid/src/Actions/MassAction.php`
- Create: `packages/Rehla/DataGrid/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Provide a safe reusable server-side DataGrid engine with registered columns, filters, sorting, pagination, actions and export contracts.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/DataGrid/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): implement security invariants as regression tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 5: Implement registered grid query pipeline

**Files:**
- Create: `packages/Rehla/DataGrid/src/GridRegistry.php`
- Create: `packages/Rehla/DataGrid/src/Query/GridQuery.php`
- Create: `packages/Rehla/DataGrid/src/DataGrid.php`
- Create: `packages/Rehla/DataGrid/tests/Feature/GridQueryTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Only registered columns/filters/sorts accepted
  - Grid registry resolves stable IDs rather than HTTP class names

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests/Feature/GridQueryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): implement registered grid query pipeline"
```

Do not include unrelated working-tree changes in the commit.

### Task 6: Run Rehla DataGrid completion gate

**Files:**
- Create: `docs/superpowers/evidence/datagrid-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/DataGrid/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-datagrid): run rehla datagrid completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/datagrid/rehla-datagrid-design.md` and map every requirement to an implemented task/test.
