# Rehla ImageCache Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate and cache whitelisted image derivatives for eligible public media without exposing filesystem paths or private documents.
...
**Spec:** `docs/superpowers/specs/image-cache/rehla-image-cache-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/image-cache/rehla-image-cache-design.md` completely.
...
- `packages/Rehla/ImageCache/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/ImageCache/src/Providers/ImageCacheServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/ImageCache/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla ImageCache package boundary

**Files:**
- Create: `packages/Rehla/ImageCache/composer.json`
- Create: `packages/Rehla/ImageCache/src/Providers/ImageCacheServiceProvider.php`
- Create: `packages/Rehla/ImageCache/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `media`
- Produces: only interfaces declared in `docs/superpowers/specs/image-cache/rehla-image-cache-design.md` for this task's responsibility.
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): bootstrap rehla imagecache package boundary"
```
...
**Files:**
- Create: `packages/Rehla/ImageCache/src/Contracts/ImageTransformer.php`
- Create: `packages/Rehla/ImageCache/src/Contracts/ImageDerivativeCache.php`
- Create: `packages/Rehla/ImageCache/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/ImageCache/src/Presets/ImagePreset.php`
- Create: `packages/Rehla/ImageCache/src/Presets/ImagePresetRegistry.php`
- Create: `packages/Rehla/ImageCache/src/Cache/ImageCacheKey.php`
- Create: `packages/Rehla/ImageCache/src/Services/ImageDerivativeService.php`
- Create: `packages/Rehla/ImageCache/src/Http/Controllers/ImageCacheController.php`
- Create: `packages/Rehla/ImageCache/src/Routes/web.php`
- Create: `packages/Rehla/ImageCache/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Generate and cache whitelisted image derivatives for eligible public media without exposing filesystem paths or private documents.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/ImageCache/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): implement security invariants as regression tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 5: Implement preset derivative route/service

**Files:**
- Create: `packages/Rehla/ImageCache/src/Presets/ImagePresetRegistry.php`
- Create: `packages/Rehla/ImageCache/src/Services/ImageDerivativeService.php`
- Create: `packages/Rehla/ImageCache/src/Http/Controllers/ImageCacheController.php`
- Create: `packages/Rehla/ImageCache/tests/Feature/ImageCacheEndpointTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Endpoint accepts preset + media UUID only
  - Private media and unknown presets rejected
  - Cache key changes with source version

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests/Feature/ImageCacheEndpointTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): implement preset derivative route/service"
```

Do not include unrelated working-tree changes in the commit.

### Task 6: Run Rehla ImageCache completion gate

**Files:**
- Create: `docs/superpowers/evidence/image-cache-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/ImageCache/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-image-cache): run rehla imagecache completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/image-cache/rehla-image-cache-design.md` and map every requirement to an implemented task/test.
