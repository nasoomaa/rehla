# Rehla Media Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Own file metadata and secure public/private storage abstractions for service media and sensitive traveller/payment documents.
...
**Spec:** `docs/superpowers/specs/media/rehla-media-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/media/rehla-media-design.md` completely.
...
- `packages/Rehla/Media/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/Media/src/Providers/MediaServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/Media/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla Media package boundary

**Files:**
- Create: `packages/Rehla/Media/composer.json`
- Create: `packages/Rehla/Media/src/Providers/MediaServiceProvider.php`
- Create: `packages/Rehla/Media/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`
- Produces: only interfaces declared in `docs/superpowers/specs/media/rehla-media-design.md` for this task's responsibility.
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): bootstrap rehla media package boundary"
```
...
**Files:**
- Create: `packages/Rehla/Media/src/Contracts/MediaRepository.php`
- Create: `packages/Rehla/Media/src/Contracts/MediaStorage.php`
- Create: `packages/Rehla/Media/src/Contracts/MediaAuthorizer.php`
- Create: `packages/Rehla/Media/src/Contracts/MalwareScanner.php`
- Create: `packages/Rehla/Media/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/Media/src/Database/Migrations/2026_08_22_040001_create_media_table.php`
- Create: `packages/Rehla/Media/src/Models/Media.php`
- Create: `packages/Rehla/Media/tests/Feature/PersistenceTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/PersistenceTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): implement package-owned persistence"
```
...
**Files:**
- Create: `packages/Rehla/Media/src/Repositories/EloquentMediaRepository.php`
- Create: `packages/Rehla/Media/src/Services/MediaUploadService.php`
- Create: `packages/Rehla/Media/src/Services/PrivateDownloadService.php`
- Create: `packages/Rehla/Media/src/Storage/MediaStorageManager.php`
- Create: `packages/Rehla/Media/src/Support/MediaUuid.php`
- Create: `packages/Rehla/Media/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Own file metadata and secure public/private storage abstractions for service media and sensitive traveller/payment documents.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/Media/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): implement security invariants as regression tests"
```
...
**Files:**
- Create: `packages/Rehla/Media/src/Events/MediaUploaded.php`
- Create: `packages/Rehla/Media/src/Events/MediaDeleted.php`
- Create: `packages/Rehla/Media/tests/Feature/EventsTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/EventsTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): implement typed events and secondary-reaction boundary"
```

Do not include unrelated working-tree changes in the commit.

### Task 7: Implement private authorized download flow

**Files:**
- Create: `packages/Rehla/Media/src/Services/PrivateDownloadService.php`
- Create: `packages/Rehla/Media/tests/Feature/PrivateDownloadTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Media UUID resolves server-side
  - Authorization checked before stream/response
  - No raw user path accepted

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests/Feature/PrivateDownloadTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): implement private authorized download flow"
```

Do not include unrelated working-tree changes in the commit.

### Task 8: Run Rehla Media completion gate

**Files:**
- Create: `docs/superpowers/evidence/media-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Media/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-media): run rehla media completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/media/rehla-media-design.md` and map every requirement to an implemented task/test.
