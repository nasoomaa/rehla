# Rehla Payment Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Own payment methods, banks, payment records/attempts, private payment proof and independently authorized payment state transitions.

**Architecture:** This plan implements one Rehla execution unit only. It consumes already-approved dependencies (core, sales, media), keeps the package boundary from its design spec, and defers downstream behavior to later plans. Every behavior-bearing task uses TDD and every task ends with fresh verification and a focused commit.
...
**Spec:** `docs/superpowers/specs/payment/rehla-payment-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/payment/rehla-payment-design.md` completely.
...
- `packages/Rehla/Payment/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/Payment/src/Providers/PaymentServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/Payment/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla Payment package boundary

**Files:**
- Create: `packages/Rehla/Payment/composer.json`
- Create: `packages/Rehla/Payment/src/Providers/PaymentServiceProvider.php`
- Create: `packages/Rehla/Payment/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`, `sales`, `media`
- Produces: only interfaces declared in `docs/superpowers/specs/payment/rehla-payment-design.md` for this task's responsibility.

- [ ] **Step 1: Write the failing test or executable gate first**

  Required behavior:
  - Package autoloads and boots only approved dependencies: `core`, `sales`, `media`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): bootstrap rehla payment package boundary"
```
...
**Files:**
- Create: `packages/Rehla/Payment/src/Contracts/PaymentRepository.php`
- Create: `packages/Rehla/Payment/src/Contracts/PaymentSubmission.php`
- Create: `packages/Rehla/Payment/src/Contracts/PaymentReview.php`
- Create: `packages/Rehla/Payment/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/Payment/src/Database/Migrations/2026_08_22_130001_create_banks_table.php`
- Create: `packages/Rehla/Payment/src/Database/Migrations/2026_08_22_130002_create_payment_methods_table.php`
- Create: `packages/Rehla/Payment/src/Database/Migrations/2026_08_22_130003_create_payments_table.php`
- Create: `packages/Rehla/Payment/src/Database/Migrations/2026_08_22_130004_create_payment_attempts_table.php`
- Create: `packages/Rehla/Payment/src/Database/Migrations/2026_08_22_130005_create_payment_status_history_table.php`
- Create: `packages/Rehla/Payment/src/Models/Bank.php`
- Create: `packages/Rehla/Payment/src/Models/PaymentMethod.php`
- Create: `packages/Rehla/Payment/src/Models/Payment.php`
- Create: `packages/Rehla/Payment/src/Models/PaymentAttempt.php`
- Create: `packages/Rehla/Payment/src/Models/PaymentStatusHistory.php`
- Create: `packages/Rehla/Payment/tests/Feature/PersistenceTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/PersistenceTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): implement package-owned persistence"
```
...
**Files:**
- Create: `packages/Rehla/Payment/src/Repositories/EloquentPaymentRepository.php`
- Create: `packages/Rehla/Payment/src/Services/PaymentSubmissionService.php`
- Create: `packages/Rehla/Payment/src/Services/PaymentReviewService.php`
- Create: `packages/Rehla/Payment/src/State/PaymentStatus.php`
- Create: `packages/Rehla/Payment/src/State/PaymentStateMachine.php`
- Create: `packages/Rehla/Payment/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Own payment methods, banks, payment records/attempts, private payment proof and independently authorized payment state transitions.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/Payment/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): implement security invariants as regression tests"
```
...
**Files:**
- Create: `packages/Rehla/Payment/src/Events/PaymentSubmitted.php`
- Create: `packages/Rehla/Payment/src/Events/PaymentApproved.php`
- Create: `packages/Rehla/Payment/src/Events/PaymentRejected.php`
- Create: `packages/Rehla/Payment/tests/Feature/EventsTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/EventsTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): implement typed events and secondary-reaction boundary"
```
...
- Create: `packages/Rehla/Payment/src/State/PaymentStatus.php`
- Create: `packages/Rehla/Payment/src/State/PaymentStateMachine.php`
- Create: `packages/Rehla/Payment/tests/Feature/StateMachineTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests/Feature/StateMachineTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): implement explicit state machine and history"
```

Do not include unrelated working-tree changes in the commit.

### Task 8: Run Rehla Payment completion gate

**Files:**
- Create: `docs/superpowers/evidence/payment-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Payment/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-payment): run rehla payment completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/payment/rehla-payment-design.md` and map every requirement to an implemented task/test.
