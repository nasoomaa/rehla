# Rehla Rule Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Provide a reusable business-neutral condition evaluation engine used later by CartRule without executable user expressions.
...
**Spec:** `docs/superpowers/specs/rule/rehla-rule-design.md`  
...
1. Read the parent spec and `docs/superpowers/specs/rule/rehla-rule-design.md` completely.
...
- `packages/Rehla/Rule/composer.json` — package metadata/dependencies/autoload
- `packages/Rehla/Rule/src/Providers/RuleServiceProvider.php` — deterministic package bootstrap
- `packages/Rehla/Rule/tests/` — package-level behavior/security/boundary tests
...
### Task 1: Bootstrap Rehla Rule package boundary

**Files:**
- Create: `packages/Rehla/Rule/composer.json`
- Create: `packages/Rehla/Rule/src/Providers/RuleServiceProvider.php`
- Create: `packages/Rehla/Rule/tests/Feature/PackageBootTest.php`

**Interfaces:**
- Consumes: approved direct dependency contracts from `core`
- Produces: only interfaces declared in `docs/superpowers/specs/rule/rehla-rule-design.md` for this task's responsibility.
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests/Feature/PackageBootTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): bootstrap rehla rule package boundary"
```
...
**Files:**
- Create: `packages/Rehla/Rule/src/Contracts/RuleContext.php`
- Create: `packages/Rehla/Rule/src/Contracts/ConditionContract.php`
- Create: `packages/Rehla/Rule/src/Contracts/OperatorContract.php`
- Create: `packages/Rehla/Rule/src/Contracts/RuleEvaluator.php`
- Create: `packages/Rehla/Rule/tests/Architecture/BoundaryTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests/Architecture/BoundaryTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): define public contracts and boundary tests"
```
...
**Files:**
- Create: `packages/Rehla/Rule/src/Conditions/FieldCondition.php`
- Create: `packages/Rehla/Rule/src/Conditions/ConditionGroup.php`
- Create: `packages/Rehla/Rule/src/Operators/OperatorRegistry.php`
- Create: `packages/Rehla/Rule/src/Evaluator.php`
- Create: `packages/Rehla/Rule/src/Results/RuleResult.php`
- Create: `packages/Rehla/Rule/tests/Feature/PrimaryBehaviorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - Exercises the primary responsibility: Provide a reusable business-neutral condition evaluation engine used later by CartRule without executable user expressions.
  - Uses only approved public dependency interfaces and preserves transaction/idempotency rules

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests/Feature/PrimaryBehaviorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): implement the smallest vertical service path"
```
...
**Files:**
- Create: `packages/Rehla/Rule/tests/Feature/SecurityInvariantTest.php`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests/Feature/SecurityInvariantTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): implement security invariants as regression tests"
```

Do not include unrelated working-tree changes in the commit.

### Task 5: Implement evaluator and nested condition groups

**Files:**
- Create: `packages/Rehla/Rule/src/Conditions/Condition.php`
- Create: `packages/Rehla/Rule/src/Conditions/ConditionGroup.php`
- Create: `packages/Rehla/Rule/src/Evaluator.php`
- Create: `packages/Rehla/Rule/tests/Unit/EvaluatorTest.php`
...
- [ ] **Step 1: Write the failing test or executable gate first**
  ...
  Required behavior:
  - ALL/ANY/nested semantics match spec
  - Unknown operator fails safely

  For behavior-bearing PHP work, create the smallest Pest test that fails because the required production behavior is absent. For configuration/generated metadata, use the first executable validation command as the red gate.

- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests/Unit/EvaluatorTest.php`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): implement evaluator and nested condition groups"
```

Do not include unrelated working-tree changes in the commit.

### Task 6: Run Rehla Rule completion gate

**Files:**
- Create: `docs/superpowers/evidence/rule-gate.md`
...
- [ ] **Step 2: Run the focused RED/gate command**
  - Run: `./vendor/bin/pest packages/Rehla/Rule/tests`
...
git diff --check
git add -p
git commit -m "feat(rehla-rule): run rehla rule completion gate"
```
...
- [ ] Re-read `docs/superpowers/specs/rule/rehla-rule-design.md` and map every requirement to an implemented task/test.
