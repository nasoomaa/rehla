...
Build the Bagisto-aligned Rehla admin presentation foundation and design system without prematurely implementing business-domain screens.

## 2. Scope Boundary

**Implementation location:** `packages/Rehla/Dashboard`  
**Direct design dependencies:** `core`, `datagrid`, `media`, `image-cache`

### Owns
- DashboardServiceProvider
- central admin menu/ACL/system config files
- admin route composition
- Blade layout/header/sidebar
- RTL/LTR
- dark mode
- reusable Blade component library
- Alpine behaviors
- admin Vite/Tailwind build
- DataGrid UI shell
- Playwright admin baseline
- safe cache-management service shell

### Explicitly does not own
- Catalog/Customer/Sales/Payment/Application business logic
...
- admin layout component API
- menu rendering contract from Core
- ACL rendering helpers
- DataGrid Blade/JS contract
- flash/modal/drawer/form component APIs
...
**Known downstream consumers:** `admin-users`, `dashboard-integration`
...
- Admin routes separated into auth/public vs protected
- Protected route ACL fail-closed infrastructure
- CSRF/session security
- CacheManager explicit command allowlist only
- No secrets rendered in configuration components
...
- menu respects ACL
- protected route without ACL mapping fails gate
- frontend production build
- Playwright login-shell/navigation smoke
