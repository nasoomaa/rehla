...
Own established orders and their lifecycle after Checkout, preserving immutable customer/service/money snapshots and independent order status.

## 2. Scope Boundary

**Implementation location:** `packages/Rehla/Sales`  
**Direct design dependencies:** `core`, `customers`, `catalog`, `checkout`
...
- OrderStatus
- OrderStatusHistory
- order lifecycle service/state machine
- cancellation
...
- OrderRepository
- OrderCreator implementing Checkout OrderCreationPort
- OrderStateMachine
- OrderQueryService
...
**Known downstream consumers:** `payment`, `applications`, `notifications`, `api`, `dashboard-integration`
...
- order creation from checkout port
- snapshot persistence
- valid/invalid state transitions
- history append
- cancellation rules
- OrderStatus independent from payment/application
