...
class ReviewDataGrid extends DataGrid
{
    ...
    public function prepareColumns()
    {
        $this->addColumn([
            ...
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.customer-names'),
            'type' => 'string',
            'sortable' => true,
        ]);
        ...
        $this->addColumn([
            'index' => 'product_name',
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.product'),
            'type' => 'string',
            'searchable' => true,
            'sortable' => true,
        ]);
        ...
        $this->addColumn([
            ...
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.status'),
            'type' => 'string',
            'filterable' => true,
            ...
            'closure' => function ($row) {
                switch ($row->product_review_status) {
                    case self::STATUS_APPROVED:
                        return '<p class="label-active">'.trans('dashboard::app.customers.reviews.index.datagrid.approved').'</p>';

                    case self::STATUS_PENDING:
                        return '<p class="label-pending">'.trans('dashboard::app.customers.reviews.index.datagrid.pending').'</p>';

                    case self::STATUS_DISAPPROVED:
                        return '<p class="label-canceled">'.trans('dashboard::app.customers.reviews.index.datagrid.disapproved').'</p>';
                }
            },
        ]);
        ...
        $this->addColumn([
            'index' => 'rating',
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.rating'),
            'type' => 'string',
            'searchable' => true,
            'filterable' => true,
            'filterable_type' => 'dropdown',
        ...
        $this->addColumn([
            ...
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.id'),
            'type' => 'integer',
            'filterable' => true,
            'sortable' => true,
        ]);
        ...
        $this->addColumn([
            'index' => 'comment',
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.comment'),
            'type' => 'string',
        ]);

        $this->addColumn([
            'index' => 'created_at',
            'label' => trans('dashboard::app.customers.reviews.index.datagrid.date'),
            'type' => 'date',
            'filterable' => true,
    ...
    public function prepareActions()
    {
        if (bouncer()->hasPermission('customers.reviews.edit')) {
            $this->addAction([
                'index' => 'edit',
                'icon' => 'icon-edit',
                'title' => trans('dashboard::app.customers.reviews.index.datagrid.edit'),
                'method' => 'GET',
                'url' => function ($row) {
        ...
        if (bouncer()->hasPermission('customers.reviews.delete')) {
            $this->addAction([
                'index' => 'delete',
                'icon' => 'icon-delete',
                'title' => trans('dashboard::app.customers.reviews.index.datagrid.delete'),
                'method' => 'DELETE',
                'url' => function ($row) {
    ...
    public function prepareMassActions()
    {
        ...
        if (bouncer()->hasPermission('customers.reviews.edit')) {
            $this->addMassAction([
                'title' => trans('dashboard::app.customers.reviews.index.datagrid.update-status'),
                'method' => 'POST',
