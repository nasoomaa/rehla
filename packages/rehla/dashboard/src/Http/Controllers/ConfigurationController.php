...
use Rehla\Dashboard\Http\Requests\ConfigurationForm;
use Rehla\Core\Repositories\CoreConfigRepository;
...
class ConfigurationController extends Controller
{
    ...
    public function index(): View
    {
        if (
            ...
            return view('dashboard::configuration.edit');
        }

        return view('dashboard::configuration.index');
    }
    ...
    public function store(ConfigurationForm $request): RedirectResponse
    {
        ...
        if (isset($data['sales']['carriers'])) {
            ...
            if (! $atLeastOneCarrierEnabled) {
                session()->flash('error', trans('dashboard::app.configuration.index.enable-at-least-one-shipping'));
            ...
            if (! $atLeastOnePaymentMethodEnabled) {
                session()->flash('error', trans('dashboard::app.configuration.index.enable-at-least-one-payment'));
        ...
        session()->flash('success', trans('dashboard::app.configuration.index.save-message'));
