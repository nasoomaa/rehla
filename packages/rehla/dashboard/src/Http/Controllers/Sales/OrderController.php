...
namespace Rehla\Dashboard\Http\Controllers\Sales;
...
use Rehla\Dashboard\DataGrids\Sales\OrderDataGrid;
use Rehla\Dashboard\Http\Controllers\Controller;
use Rehla\Dashboard\Http\Resources\AddressResource;
use Rehla\Dashboard\Http\Resources\CartResource;
...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dashboard::sales.orders.index', compact('channels', 'groups'));
    }
    ...
    public function create(int $cartId)
    {
        ...
        return view('dashboard::sales.orders.create', compact('cart', 'addresses'));
    }
    ...
    public function store(int $cartId)
    {
        ...
        if (Cart::hasError()) {
            return response()->json([
                'message' => trans('dashboard::app.sales.orders.create.error'),
        ...
        if (! in_array($cart->payment->method, ['cashondelivery', 'moneytransfer'])) {
            return response()->json([
                'message' => trans('dashboard::app.sales.orders.create.payment-not-supported'),
            ], Response::HTTP_BAD_REQUEST);
        }
        ...
        session()->flash('order', trans('dashboard::app.sales.orders.create.order-placed-success'));
    ...
    public function view(int $id)
    {
        ...
        return view('dashboard::sales.orders.view', compact('order'));
    }
    ...
    public function reorder(int $id)
    {
        ...
        if (! $order->customer) {
            session()->flash('error', trans('dashboard::app.sales.orders.view.reorder-customer-missing'));
        ...
        if ($skippedBooking) {
            session()->flash('info', trans('dashboard::app.sales.orders.view.reorder-booking-skipped'));
        }
    ...
    public function cancel(int $id)
    {
        ...
        if ($result) {
            session()->flash('success', trans('dashboard::app.sales.orders.view.cancel-success'));
        } else {
            session()->flash('error', trans('dashboard::app.sales.orders.view.create-error'));
        }
    ...
    public function comment(int $id)
    {
        ...
        session()->flash('success', trans('dashboard::app.sales.orders.view.comment-success'));
    ...
    public function validateOrder()
    {
        ...
        if (! Cart::haveMinimumOrderAmount()) {
            throw new \Exception(trans('dashboard::app.sales.orders.create.minimum-order-error', [
        ...
        if (
            ...
            throw new \Exception(trans('dashboard::app.sales.orders.create.check-shipping-address'));
        }

        if (! $cart->billing_address) {
            throw new \Exception(trans('dashboard::app.sales.orders.create.check-billing-address'));
        }
        ...
        if (
            ...
            throw new \Exception(trans('dashboard::app.sales.orders.create.specify-shipping-method'));
        }

        if (! $cart->payment) {
            throw new \Exception(trans('dashboard::app.sales.orders.create.specify-payment-method'));
        }
    }
}
