...
namespace Rehla\Dashboard\Http\Controllers\Sales\RMA;
...
class RequestController extends Controller
{
    ...
    public function index(): View|JsonResponse
    {
        ...
        return view('dashboard::sales.rma.returns.index');
    }
    ...
    public function view(int $rmaId): View|RedirectResponse
    {
        ...
        return view('dashboard::sales.rma.returns.view', compact('rma', 'statusArray', 'canReopenRma'));
    }
    ...
    public function reOpenRequest(int $id): RedirectResponse
    {
        ...
        if (! $this->rmaRepository->canReopenRma($rma)) {
            session()->flash('error', trans('dashboard::app.sales.rma.all-rma.view.reopen-not-allowed'));
        ...
        if (! empty($data['close_rma'])) {
            ...
            $this->rmaMessageRepository->create([
                'message' => trans('dashboard::app.sales.rma.all-rma.view.conversation-process'),
                'rma_id' => $id,
                'is_admin' => 1,
        ...
        session()->flash('success', trans('dashboard::app.sales.rma.all-rma.view.update-success'));
    ...
    public function sendMessage(): JsonResponse
    {
        ...
        if (! empty($storedMessage)) {
            ...
            return new JsonResponse([
                'message' => trans('dashboard::app.sales.rma.all-rma.view.send-message-success'),
            ]);
        }

        return new JsonResponse([
            'message' => trans('dashboard::app.sales.rma.all-rma.view.failed'),
        ]);
    }
    ...
    public function create(): View|JsonResponse
    {
        ...
        return view('dashboard::sales.rma.returns.create');
    }
    ...
    public function store(): RedirectResponse|JsonResponse
    {
        ...
        if ($rma->item) {
            ...
            return response()->json([
                'messages' => trans('dashboard::app.sales.rma.create-rma.create-success'),
                'redirect_url' => route('admin.sales.rma.requests.view', $rma->id),
            ]);
        }

        return response()->json([
            'messages' => trans('dashboard::app.sales.rma.create-rma.failed'),
    ...
    private function handleReceivedPackage($rma, array $data): JsonResponse
    {
        try {
            ...
            if (! $this->processOrderRefund($rma->order, $refundData)) {
                throw new \Exception(trans('dashboard::app.sales.rma.all-rma.view.refund-failed'));
            }
    ...
    {
        if (! $order->canRefund()) {
            session()->flash('error', trans('dashboard::app.sales.refunds.create.creation-error'));
        ...
        try {
            ...
            if (! $totals) {
                throw new InvalidRefundQuantityException(
                    trans('dashboard::app.sales.refunds.create.invalid-qty')
                );
            }
    ...
    private function validateRefundAmount($order, array $totals, array $refundData): array
    {
        ...
        if (! $refundAmount) {
            return [
                'valid' => false,
                'message' => trans('dashboard::app.sales.refunds.create.invalid-refund-amount-error'),
            ];
        }
        ...
        if ($refundAmount > $maxRefundAmount) {
            return [
                'valid' => false,
                'message' => trans('dashboard::app.sales.refunds.create.refund-limit-error', [
    ...
    private function finalizeRmaUpdate($rma, array $data): JsonResponse
    {
        ...
        $this->rmaMessageRepository->create([
            'message' => trans('dashboard::app.sales.rma.all-rma.view.status-message', [
                'id' => $rma->id,
        ...
        return new JsonResponse([
            'messages' => trans('dashboard::app.sales.rma.all-rma.view.update-success'),
        ]);
    }
}
