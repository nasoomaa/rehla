...
use Rehla\Dashboard\DataGrids\Sales\OrderTransactionDataGrid;
use Rehla\Dashboard\Http\Controllers\Controller;
use Rehla\Dashboard\Http\Resources\TransactionResource;
...
class TransactionController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dashboard::sales.transactions.index', compact('paymentMethods'));
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $invoice) {
            return new JsonResponse([
                'message' => trans('dashboard::app.sales.transactions.index.create.invoice-missing'),
            ], 400);
        }
        ...
        if ($invoice->state == 'paid') {
            return new JsonResponse([
                'message' => trans('dashboard::app.sales.transactions.index.create.already-paid'),
            ], 400);
        }

        if ($transactionAmtFinal > $invoice->base_grand_total) {
            return new JsonResponse([
                'message' => trans('dashboard::app.sales.transactions.index.create.transaction-amount-exceeds'),
            ], 400);
        }

        if ($request->amount <= 0) {
            return new JsonResponse([
                'message' => trans('dashboard::app.sales.transactions.index.create.transaction-amount-zero'),
            ], 400);
        }
        ...
        return new JsonResponse([
            'message' => trans('dashboard::app.sales.transactions.index.create.transaction-saved'),
        ]);
    }
