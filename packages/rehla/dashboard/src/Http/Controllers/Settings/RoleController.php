...
use Rehla\Dashboard\DataGrids\Settings\RolesDataGrid;
...
class RoleController extends Controller
{
    ...
    public function store()
    {
        ...
        session()->flash('success', trans('dashboard::app.settings.roles.create-success'));
    ...
    public function update(int $id)
    {
        ...
        if (
            ...
            session()->flash('error', trans('dashboard::app.settings.roles.being-used'));
        ...
        session()->flash('success', trans('dashboard::app.settings.roles.update-success'));
    ...
    public function destroy(int $id): JsonResponse
    {
        ...
        if ($role->admins->count() >= 1) {
            return new JsonResponse(['message' => trans('dashboard::app.settings.roles.being-used', [
                'name' => 'dashboard::app.settings.roles.index.title',
                'source' => 'dashboard::app.settings.roles.index.admin-user',
            ])], 400);
        }
        ...
        if ($this->roleRepository->count() == 1) {
            return new JsonResponse([
                'message' => trans(
                    'dashboard::app.settings.roles.last-delete-error'
                ),
            ], 400);
        }
        ...
        try {
            ...
            return new JsonResponse(['message' => trans('dashboard::app.settings.roles.delete-success')]);
        } catch (\Exception $e) {
        }

        return new JsonResponse([
            'message' => trans(
                'dashboard::app.settings.roles.delete-failed'
            ),
        ], 500);
    }
}
