...
use Rehla\Dashboard\DataGrids\Settings\UserDataGrid;
use Rehla\Dashboard\Http\Controllers\Controller;
use Rehla\Dashboard\Http\Requests\UserForm;
...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dashboard::settings.users.index', compact('roles'));
    }
    ...
    public function store(UserForm $request): JsonResponse
    {
        ...
        return new JsonResponse([
            'message' => trans('dashboard::app.settings.users.create-success'),
        ]);
    }
    ...
    public function update(UserForm $request): JsonResponse
    {
        ...
        if ($data instanceof RedirectResponse) {
            return new JsonResponse([
                'message' => trans('dashboard::app.settings.users.update-success'),
            ]);
        }
    ...
    public function destroy($id): JsonResponse
    {
        if ($this->adminRepository->count() == 1) {
            return new JsonResponse([
                'message' => trans('dashboard::app.settings.users.last-delete-error'),
            ], 400);
        }

        if (auth()->guard('admin')->user()->id == $id) {
            return new JsonResponse([
                'message' => trans('dashboard::app.settings.users.delete-self-error'),
            ], 403);
        }
        ...
        try {
            ...
            return new JsonResponse([
                'message' => trans('dashboard::app.settings.users.delete-success'),
            ], 200);
        } catch (\Exception $e) {
        }

        return new JsonResponse([
            'message' => trans('dashboard::app.settings.users.delete-failed'),
        ], 500);
    }
    ...
    public function confirm($id)
    {
        ...
        return view('dashboard::customers.customers.confirm-password', compact('user'));
    }
    ...
    public function destroySelf(): JsonResponse
    {
        ...
        if (Hash::check($password, auth()->guard('admin')->user()->password)) {
            if ($this->adminRepository->count() == 1) {
                session()->flash('error', trans('dashboard::app.settings.users.delete-last'));
            } else {
            ...
            return new JsonResponse([
                'message' => trans('dashboard::app.settings.users.incorrect-password'),
            ], 404);
        }
    }
    ...
    {
        session()->flash('error', trans('dashboard::app.settings.users.cannot-change', [
            'name' => $columnName,
        ]));
