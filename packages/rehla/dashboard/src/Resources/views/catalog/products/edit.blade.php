<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.catalog.products.edit.title')
    </x-slot>

    {!! view_render_event('rehla.dashboard.catalog.product.edit.before', ['product' => $product]) !!}

    <x-dashboard::form
        method="PUT"
        enctype="multipart/form-data"
    >
        {!! view_render_event('rehla.dashboard.catalog.product.edit.actions.before', ['product' => $product]) !!}
                ...
                <div class="flex items-center gap-x-2.5">
                    ...
                    @if (
                        ...
                        <a
                            ...
                            target="_blank"
                        >
                            @lang('dashboard::app.catalog.products.edit.preview')
                        </a>
                    @endif

                    <!-- Save Button -->
                    <button class="primary-button">
                        @lang('dashboard::app.catalog.products.edit.save-btn')
                    </button>
                </div>
            </div>
        </div>
        ...
        {!! view_render_event('rehla.dashboard.catalog.product.edit.actions.after', ['product' => $product]) !!}

        <!-- body content -->
        {!! view_render_event('rehla.dashboard.catalog.product.edit.form.before', ['product' => $product]) !!}
        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            @foreach ($groupedColumns as $column => $groups)
                {!! view_render_event("rehla.dashboard.catalog.product.edit.form.column_{$column}.before", ['product' => $product]) !!}
                ...
                <div class="flex flex-col gap-2 {{ $column == 1 ? 'flex-1 max-xl:flex-auto' : 'w-[360px] max-w-full max-sm:w-full' }}">
                    @foreach ($groups as $group)
                        ...
                        @if ($customAttributes->isNotEmpty())
                            {!! view_render_event("rehla.dashboard.catalog.product.edit.form.{$group->code}.before", ['product' => $product]) !!}
                            ...
                            <div class="box-shadow relative rounded bg-white p-4 dark:bg-gray-900">
                                ...
                                @foreach ($customAttributes as $attribute)
                                    {!! view_render_event("rehla.dashboard.catalog.product.edit.form.{$group->code}.controls.before", ['product' => $product]) !!}
                                    ...
                                    <x-dashboard::form.control-group class="last:!mb-0">
                                        ...
                                        <x-dashboard::form.control-group.error :control-name="$attribute->code . (in_array($attribute->type, ['multiselect', 'checkbox']) ? '[]' : '')" />
                                    </x-dashboard::form.control-group>

                                    {!! view_render_event("rehla.dashboard.catalog.product.edit.form.{$group->code}.controls.after", ['product' => $product]) !!}
                                @endforeach
                            ...
                            {!! view_render_event("rehla.dashboard.catalog.product.edit.form.{$group->code}.after", ['product' => $product]) !!}
                        @endif
                    @endforeach

                    @if ($column == 1)
                        <!-- Images View Blade File -->
                        @include('dashboard::catalog.products.edit.images')

                        <!-- Videos View Blade File -->
                        @include('dashboard::catalog.products.edit.videos')
                        ...
                        @include('dashboard::catalog.products.edit.links')
                    ...
                    @elseif (! $isSingleColumn)
                        ...
                        @include('dashboard::catalog.products.edit.channels')

                        <!-- Categories View Blade File -->
                        @include('dashboard::catalog.products.edit.categories')
                    @endif
                </div>
                ...
                {!! view_render_event("rehla.dashboard.catalog.product.edit.form.column_{$column}.after", ['product' => $product]) !!}
            @endforeach
        </div>

        {!! view_render_event('rehla.dashboard.catalog.product.edit.form.after', ['product' => $product]) !!}
    </x-dashboard::form>

    {!! view_render_event('rehla.dashboard.catalog.product.edit.after', ['product' => $product]) !!}
</x-dashboard::layouts>
