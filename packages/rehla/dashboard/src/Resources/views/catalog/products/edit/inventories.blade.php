{!! view_render_event('rehla.dashboard.catalog.product.edit.form.inventories.controls.before', ['product' => $product]) !!}
...
<v-inventories>
    ...
    <div class="mb-5 text-sm text-gray-600 dark:text-gray-300">
        <div class="relative mb-2.5 flex items-center">
            ...
            @lang('dashboard::app.catalog.products.edit.inventories.pending-ordered-qty', [
            ...
            <div class="absolute bottom-6 hidden rounded-lg bg-black p-2.5 text-sm italic text-white opacity-80 peer-hover:block">
                @lang('dashboard::app.catalog.products.edit.inventories.pending-ordered-qty-info')
            </div>
        </div>
    </div>
    ...
    @foreach (app(\Webkul\Inventory\Repositories\InventorySourceRepository::class)->findWhere(['status' => 1]) as $inventorySource)
        ...
        <x-dashboard::form.control-group>
            ...
            <x-dashboard::form.control-group.error :control-name="'inventories[' . $inventorySource->id . ']'" />
        </x-dashboard::form.control-group>
    @endforeach
</v-inventories>

{!! view_render_event('rehla.dashboard.catalog.product.edit.form.inventories.controls.after', ['product' => $product]) !!}
