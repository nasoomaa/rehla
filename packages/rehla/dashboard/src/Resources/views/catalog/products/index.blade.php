...
<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.catalog.products.index.title')
    </x-slot>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        ...
        <div class="flex items-center gap-x-2.5">
            <!-- Export Modal -->
            <x-dashboard::datagrid.export :src="route('admin.catalog.products.index')" />

            {!! view_render_event('rehla.dashboard.catalog.products.create.before') !!}
            ...
            @if (bouncer()->hasPermission('catalog.products.create'))
                <v-create-product-form>
                    ...
                    >
                        @lang('dashboard::app.catalog.products.index.create-btn')
                    </button>
                </v-create-product-form>
            @endif

            {!! view_render_event('rehla.dashboard.catalog.products.create.after') !!}
        </div>
    </div>

    {!! view_render_event('rehla.dashboard.catalog.products.list.before') !!}
                            ...
                            <div class="flex items-start gap-2.5">
                                    ...
                                    <template v-else>
                                        <div class="relative h-12 w-12 rounded border border-dashed border-gray-300 dark:border-gray-800 dark:mix-blend-exclusion dark:invert sm:h-16 sm:w-16">
                                            ...
                                            <p class="absolute bottom-0 w-full text-center text-[6px] font-semibold text-gray-400">
                                                @lang('dashboard::app.catalog.products.index.datagrid.product-image')
                                            </p>
                                        </div>
                                    </template>
                                </div>
                                ...
                                <div class="flex flex-col gap-1 flex-1">
                                    ...
                                    <p class="text-xs text-gray-600 dark:text-gray-300 sm:text-sm">
                                        @{{ "@lang('dashboard::app.catalog.products.index.datagrid.id-value')".replace(':id', record.product_id) }}
                                    </p>

                                    <p class="text-xs text-gray-600 dark:text-gray-300 sm:text-sm">
                                        @{{ "@lang('dashboard::app.catalog.products.index.datagrid.sku-value')".replace(':sku', record.sku) }}
                                    </p>

                                    <p class="text-xs text-gray-600 dark:text-gray-300 sm:text-sm">
                                        @{{ "@lang('dashboard::app.catalog.products.index.datagrid.attribute-family-value')".replace(':attribute_family', record.attribute_family) }}
                                    </p>
                                        ...
                                        <div v-else-if="{!! $asJsArray($stockDisabledTypes) !!}.includes(record.type) || ! Number(record.manage_stock)">
                                            <p class="text-xs text-gray-600 dark:text-gray-300 sm:text-sm">
                                                <span class="text-gray-500 dark:text-gray-400">
                                                    @lang('dashboard::app.catalog.products.index.datagrid.stock-disabled')
                                                </span>
                                            </p>
                                        </div>
                                        ...
                                        <div v-else>
                                            ...
                                            >
                                                <span class="text-green-600">
                                                    @{{ "@lang('dashboard::app.catalog.products.index.datagrid.qty-value')".replace(':qty', record.quantity) }}
                                                </span>
                                            </p>
                                            ...
                                            >
                                                <span class="text-red-600">
                                                    @lang('dashboard::app.catalog.products.index.datagrid.out-of-stock')
                                                </span>
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <p :class="[record.status ? 'label-active': 'label-info']">
                                        @{{ record.status ? "@lang('dashboard::app.catalog.products.index.datagrid.active')" : "@lang('dashboard::app.catalog.products.index.datagrid.disable')" }}
                                    </p>
                                </div>
                            </div>
    ...
    {!! view_render_event('rehla.dashboard.catalog.products.list.after') !!}
            ...
            <div>
                ...
                >
                    <form @submit="handleSubmit($event, create)">
                        ...
                        <x-dashboard::modal ref="productCreateModal">
                            <!-- Modal Header -->
                            <x-slot:header>
                                <p
                                ...
                                >
                                    @lang('dashboard::app.catalog.products.index.create.title')
                                </p>
                                ...
                                <p
                                    ...
                                    v-else
                                >
                                    @lang('dashboard::app.catalog.products.index.create.configurable-attributes')
                                </p>
                            </x-slot>
                            ...
                            <x-slot:content>
                                <div v-show="! attributes.length">
                                    {!! view_render_event('rehla.dashboard.catalog.products.create_form.general.controls.before') !!}

                                    <!-- Product Type -->
                                    <x-dashboard::form.control-group>
                                        <x-dashboard::form.control-group.label class="required">
                                            @lang('dashboard::app.catalog.products.index.create.type')
                                        ...
                                        <x-dashboard::form.control-group.control
                                            ...
                                            name="type"
                                            rules="required"
                                            :label="trans('dashboard::app.catalog.products.index.create.type')"
                                        >
                                    ...
                                    <x-dashboard::form.control-group>
                                        <x-dashboard::form.control-group.label class="required">
                                            @lang('dashboard::app.catalog.products.index.create.family')
                                        </x-dashboard::form.control-group.label>
                                        ...
                                        <x-dashboard::form.control-group.control
                                            ...
                                            rules="required"
                                            :label="trans('dashboard::app.catalog.products.index.create.family')"
                                        >
                                        ...
                                        <x-dashboard::form.control-group.error control-name="attribute_family_id" />
                                    </x-dashboard::form.control-group>
                                    ...
                                    <x-dashboard::form.control-group>
                                        ...
                                        <x-dashboard::form.control-group.control
                                            ...
                                            :label="trans('dashboard::app.catalog.products.index.create.sku')"
                                        />

                                        <x-dashboard::form.control-group.error control-name="sku" />
                                    </x-dashboard::form.control-group>

                                    {!! view_render_event('rehla.dashboard.catalog.products.create_form.general.controls.after') !!}
                                </div>

                                <div v-show="attributes.length">
                                    {!! view_render_event('rehla.dashboard.catalog.products.create_form.attributes.controls.before') !!}
                                    ...
                                    {!! view_render_event('rehla.dashboard.catalog.products.create_form.attributes.controls.after') !!}
                                </div>
                            </x-slot>
                            ...
                            <x-slot:footer>
                                <div class="flex items-center gap-x-2.5">
                                    ...
                                    <x-dashboard::button
                                        ...
                                        :title="trans('dashboard::app.catalog.products.index.create.back-btn')"
                                        v-if="attributes.length"
                                        @click="attributes = []"
                                    />
                                    ...
                                    <x-dashboard::button
                                        button-type="button"
                                        class="primary-button"
                                        :title="trans('dashboard::app.catalog.products.index.create.save-btn')"
                                        ::loading="isLoading"
                                        ::disabled="isLoading"
                                    />
                                </div>
