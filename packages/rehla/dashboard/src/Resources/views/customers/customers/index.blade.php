<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.customers.customers.index.title')
    </x-slot>
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-x-2.5">
            <!-- Export Modal -->
            <x-dashboard::datagrid.export src="{{ route('admin.customers.customers.index') }}" />

            <div class="flex items-center gap-x-2.5">
                <!-- Included customer create blade file -->
                @if (bouncer()->hasPermission('customers.customers.create'))
                    {!! view_render_event('rehla.dashboard.customers.customers.create.before') !!}

                    @include('dashboard::customers.customers.index.create')
                    ...
                    {!! view_render_event('rehla.dashboard.customers.customers.create.after') !!}

                    <button
                        class="primary-button"
                        @click="$refs.createCustomerComponent.openModal()"
                    >
                        @lang('dashboard::app.customers.customers.index.create.create-btn')
                    </button>
                @endif
            </div>
        </div>
    ...
    {!! view_render_event('rehla.dashboard.customers.customers.list.before') !!}
                ...
                >
                    ...
                    <div class="flex flex-col gap-1.5 ps-8 md:ps-0">
                        <div class="flex gap-1.5">
                            ...
                            >
                                @{{ record.status ? '@lang('dashboard::app.customers.customers.index.datagrid.active')' : '@lang('dashboard::app.customers.customers.index.datagrid.inactive')' }}
                            </span>
                            ...
                            >
                                @{{ record.is_suspended ?  '@lang('dashboard::app.customers.customers.index.datagrid.suspended')' : '' }}
                            </span>
                        </div>
                        ...
                        <p class="text-gray-600 dark:text-gray-300">
                            @lang('dashboard::app.customers.customers.index.datagrid.channel') - @{{ record.channel_id ? record.channel_id : 'N/A' }}
                        </p>

                        <p class="text-gray-600 dark:text-gray-300">
                            @{{ "@lang('dashboard::app.customers.customers.index.datagrid.id-value')".replace(':id', record.customer_id) }}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-between gap-x-4 ps-8 md:ps-0">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <p class="text-gray-600 dark:text-gray-300">
                                @{{ "@lang('dashboard::app.customers.customers.index.datagrid.order')".replace(':order', record.order_count) }}
                            </p>

                            <p class="text-gray-600 dark:text-gray-300">
                                @{{ "@lang('dashboard::app.customers.customers.index.datagrid.address')".replace(':address', record.address_count) }}
                            </p>
                        </div>
    ...
    {!! view_render_event('rehla.dashboard.customers.customers.list.after') !!}
</x-dashboard::layouts>
