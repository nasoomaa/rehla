...
<div class="box-shadow rounded bg-white p-4 last:pb-0 dark:bg-gray-900">
    <p class="p-4 pb-0 text-base font-semibold leading-none text-gray-800 dark:text-white">
        @lang('dashboard::app.customers.customers.view.notes.add-note')
    </p>

    <x-dashboard::form :action="route('admin.customer.note.store', $customer->id)">
        <div class="border-b p-4 dark:border-gray-800">
            ...
            <x-dashboard::form.control-group>
                <x-dashboard::form.control-group.control
                    ...
                    name="note"
                    rules="required"
                    :label="trans('dashboard::app.customers.customers.view.notes.note')"
                    :placeholder="trans('dashboard::app.customers.customers.view.notes.note-placeholder')"
                    rows="3"
                />

                <x-dashboard::form.control-group.error control-name="note" />
            </x-dashboard::form.control-group>
            ...
            <div class="flex items-center justify-between">
                ...
                >
                    ...
                    <p class="flex cursor-pointer items-center gap-x-1 font-semibold text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100">
                        @lang('dashboard::app.customers.customers.view.notes.notify-customer')
                    </p>
                </label>
                ...
                >
                    @lang('dashboard::app.customers.customers.view.notes.submit-btn-title')
                </button>
            </div>
        </div>
    </x-dashboard::form>
        ...
        <div class="grid gap-1.5 border-b p-4 last:border-none dark:border-gray-800">
            ...
            <p class="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                @if ($note->customer_notified)
                    ...
                    @lang('dashboard::app.customers.customers.view.notes.customer-notified', ['date' => core()->formatDate($note->created_at, 'Y-m-d H:i:s a')])
                @else
                    <span class="icon-cancel-1 h-fit rounded-full bg-red-100 text-2xl text-red-600 dark:!text-red-600"></span>

                    @lang('dashboard::app.customers.customers.view.notes.customer-not-notified', ['date' => core()->formatDate($note->created_at, 'Y-m-d H:i:s a')])
                @endif
            </p>
        </div>
    @endforeach
