...
<v-dashboard-stock-threshold-products>
    <!-- Shimmer -->
    <x-dashboard::shimmer.dashboard.stock-threshold-products />
</v-dashboard-stock-threshold-products>
                        ...
                        <div class="flex gap-2.5">
                            ...
                            <template v-else>
                                <div class="relative h-[65px] max-h-[65px] w-full max-w-[65px] overflow-hidden rounded border border-dashed border-gray-300 dark:border-gray-800 dark:mix-blend-exclusion dark:invert">
                                    ...
                                    <p class="absolute bottom-1.5 w-full text-center text-[6px] font-semibold text-gray-400">
                                        @lang('dashboard::app.dashboard.index.product-image')
                                    </p>
                                </div>
                            </template>
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <p class="text-gray-600 dark:text-gray-300">
                                    @{{ "@lang('dashboard::app.dashboard.index.sku', ['sku' => ':replace'])".replace(':replace', product.sku) }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-1.5">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <p :class="[product.total_qty > {{ core()->getConfigData('catalog.inventory.stock_options.out_of_stock_threshold') }} ? 'text-emerald-500' : 'text-red-500']">
                                    @{{ "@lang('dashboard::app.dashboard.index.total-stock', ['total_stock' => ':replace'])".replace(':replace', product.total_qty) }}
                                </p>
                            </div>
                ...
                <div class="grid justify-center justify-items-center gap-3.5 px-2.5 py-10">
                    ...
                    <div class="flex flex-col items-center">
                        <p class="text-base font-semibold text-gray-400">
                            @lang('dashboard::app.dashboard.index.empty-threshold')
                        </p>
    
                        <p class="text-gray-400">
                            @lang('dashboard::app.dashboard.index.empty-threshold-description')
                        </p>
                    </div>
                </div>
            </div>
