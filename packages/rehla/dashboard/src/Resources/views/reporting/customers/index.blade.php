<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.reporting.customers.index.title')
    </x-slot>
    ...
    <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
        <!-- Customers Section -->
        @include('dashboard::reporting.customers.total-customers')
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('dashboard::reporting.customers.most-sales')

            <!-- Customers With Most Orders Section -->
            @include('dashboard::reporting.customers.most-orders')
        </div>

        <!-- Top Customer Groups Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Top Customer Groups Section -->
            @include('dashboard::reporting.customers.top-customer-groups')

            <!-- Customer with Most Reviews Section -->
            @include('dashboard::reporting.customers.most-reviews')
        </div>
    </div>
            ...
            <div class="flex gap-1.5">
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        ...
                        placeholder="@lang('dashboard::app.reporting.customers.index.start-date')"
                    />
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400"
                        v-model="filters.end"
                        placeholder="@lang('dashboard::app.reporting.customers.index.end-date')"
                    />
                </x-dashboard::flat-picker.date>
            </div>
        </script>
            ...
            app.component('v-reporting-filters', {
                ...
                data() {
                    return {
                        channels: [
                            {
                                name: "@lang('dashboard::app.reporting.customers.index.all-channels')",
                                code: ''
                            },
