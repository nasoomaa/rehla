<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.reporting.products.index.title')
    </x-slot>
    ...
    <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('dashboard::reporting.products.sold-quantities')

            <!-- Products Added to Wishlist Section -->
            @include('dashboard::reporting.products.wishlist-products')
        </div>
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('dashboard::reporting.products.top-selling-by-revenue')

            <!-- Top Selling Products By Quantity Section -->
            @include('dashboard::reporting.products.top-selling-by-quantity')
        </div>

        <!-- Products With Most Reviews Section -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Products With Most Reviews Section -->
            @include('dashboard::reporting.products.most-reviews')
        </div>

        <!-- Last Search Terms and Top Search Terms Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Last Search Terms Section -->
            @include('dashboard::reporting.products.last-search-terms')

            <!-- Top Search Terms Section -->
            @include('dashboard::reporting.products.top-search-terms')
        </div>
    </div>
            ...
            <div class="flex gap-1.5">
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        ...
                        placeholder="@lang('dashboard::app.reporting.products.index.start-date')"
                    />
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400"
                        v-model="filters.end"
                        placeholder="@lang('dashboard::app.reporting.products.index.end-date')"
                    />
                </x-dashboard::flat-picker.date>
            </div>
        </script>
            ...
            app.component('v-reporting-filters', {
                ...
                data() {
                    return {
                        channels: [
                            {
                                name: "@lang('dashboard::app.reporting.products.index.all-channels')",
                                code: ''
                            },
