<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.reporting.sales.index.title')
    </x-slot>
    ...
    <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
        <!-- Sales Section -->
        @include('dashboard::reporting.sales.total-sales')
        ...
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            ...
            @include('dashboard::reporting.sales.purchase-funnel')

            <!-- Abandoned Carts Section -->
            @include('dashboard::reporting.sales.abandoned-carts')
        </div>

        <!-- Total Orders and Average Order Value Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Total Orders Section -->
            @include('dashboard::reporting.sales.total-orders')

            <!-- Average Order Value Section -->
            @include('dashboard::reporting.sales.average-order-value')
        </div>

        <!-- Tax Collected and Shipping Collected Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Tax Collected Section -->
            @include('dashboard::reporting.sales.tax-collected')

            <!-- Shipping Collected Section -->
            @include('dashboard::reporting.sales.shipping-collected')
        </div>

        <!-- Refunds and Top Payment Methods Sections Container -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            <!-- Refunds Section -->
            @include('dashboard::reporting.sales.total-refunds')

            <!-- Top Payment Methods Section -->
            @include('dashboard::reporting.sales.top-payment-methods')
        </div>

        <!-- Sales By Coupon Section -->
        <div class="flex flex-col justify-between gap-4 flex-1 [&>*]:flex-1 md:flex-row">
            @include('dashboard::reporting.sales.sales-by-coupon')
        </div>
    </div>
            ...
            <div class="flex gap-1.5">
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        ...
                        placeholder="@lang('dashboard::app.reporting.sales.index.start-date')"
                    />
                ...
                <x-dashboard::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-gray-600 transition-all hover:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-gray-400"
                        v-model="filters.end"
                        placeholder="@lang('dashboard::app.reporting.sales.index.end-date')"
                    />
                </x-dashboard::flat-picker.date>
            </div>
        </script>
            ...
            app.component('v-reporting-filters', {
                ...
                data() {
                    return {
                        channels: [
                            {
                                name: "@lang('dashboard::app.reporting.sales.index.all-channels')",
                                code: ''
                            },
