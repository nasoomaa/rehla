<x-dashboard::layouts>
    <!-- Page Title -->
    <x-slot:title>
        @lang('dashboard::app.sales.orders.index.title')
    </x-slot>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        ...
        <div class="flex items-center gap-x-2.5">
            <x-dashboard::datagrid.export src="{{ route('admin.sales.orders.index') }}" />

            {!! view_render_event('rehla.dashboard.sales.orders.create.before') !!}
            ...
            @if (bouncer()->hasPermission('sales.orders.create'))
                ...
                >
                    @lang('dashboard::app.sales.orders.index.create-btn')
                </button>
            @endif

            {!! view_render_event('rehla.dashboard.sales.orders.create.after') !!}
        </div>
    </div>

    <v-customer-search ref="selectCustomerComponent"></v-customer-search>

    <x-dashboard::datagrid :src="route('admin.sales.orders.index')" :isMultiRow="true">
        <template #header="{
            isLoading,
            available,
            applied,
                ...
                >
                    ...
                    <div class="flex flex-col gap-1.5">
                        <p class="text-sm sm:text-base font-semibold text-gray-800 dark:text-white">
                            @{{ "@lang('dashboard::app.sales.orders.index.datagrid.id')".replace(':id', record.increment_id) }}
                        </p>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                            @lang('dashboard::app.sales.orders.index.datagrid.pay-by', ['method' => ''])@{{ record.method }}
                        </p>
                    ...
                    <x-slot:header>
                        <div class="grid gap-3">
                            <p class="py-2 text-xl font-medium dark:text-white">
                                @lang('dashboard::app.sales.orders.index.search-customer.title')
                            </p>
                            ...
                            <div class="relative w-full">
                                <input
                                    ...
                                    placeholder="@lang('dashboard::app.sales.orders.index.search-customer.search-by')"
                        ...
                        >
                            ...
                            <div class="flex flex-col items-center gap-1.5">
                                <p class="text-base font-semibold text-gray-400">
                                    @lang('dashboard::app.sales.orders.index.search-customer.empty-title')
                                </p>

                                <p class="text-gray-400">
                                    @lang('dashboard::app.sales.orders.index.search-customer.empty-info')
                                </p>
                                ...
                                >
                                    @lang('dashboard::app.sales.orders.index.search-customer.create-btn')
                                </button>
                            </div>
                        </div>
                    </x-slot>
