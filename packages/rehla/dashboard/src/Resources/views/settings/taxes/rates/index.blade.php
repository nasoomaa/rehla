<x-dashboard::layouts>
    <x-slot:title>
        @lang('dashboard::app.settings.taxes.rates.index.title')
    </x-slot>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        ...
        <div class="flex items-center gap-x-2.5">
            <!-- Tax Rate Export -->
            <x-dashboard::datagrid.export src="{{ route('admin.settings.taxes.rates.index') }}" />
            ...
            @if (bouncer()->hasPermission('settings.taxes.tax_rates.create'))
                <a href="{{ route('admin.settings.taxes.rates.create') }}" class="primary-button">
                    @lang('dashboard::app.settings.taxes.rates.index.button-title')
                </a>
            @endif
        </div>
    </div>
