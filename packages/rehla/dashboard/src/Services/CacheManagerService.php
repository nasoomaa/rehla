...
namespace Rehla\Dashboard\Services;
...
class CacheManagerService
{
    ...
    public function execute(string $action): array
    {
        ...
        if (! isset($allActions[$action])) {
            return [
                'success' => false,
                'message' => trans('dashboard::app.configuration.index.cache-management.invalid-action'),
                'output' => '',
                'command' => '',
            ];
        }
        ...
        try {
            ...
            if ($exitCode !== 0) {
                return [
                    'success' => false,
                    'message' => trans('dashboard::app.configuration.index.cache-management.action-failed', ['action' => $command]),
            ...
            return [
                'success' => true,
                'message' => trans('dashboard::app.configuration.index.cache-management.action-success', ['action' => $command]),
                'output' => trim($rawOutput),
                'command' => $command,
            ];
        } catch (\Throwable $e) {
            return [
                'success' => false,
                'message' => trans('dashboard::app.configuration.index.cache-management.action-exception', ['message' => $e->getMessage()]),
                'output' => $e->getMessage(),
                'command' => $command,
            ];
        }
