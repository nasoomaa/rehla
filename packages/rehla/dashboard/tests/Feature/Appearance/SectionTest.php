...
it('should returns the section index page', function () {
    ...
    get(route('admin.appearance.sections.index', ['code' => core()->getCurrentChannel()->theme]))
        ->assertOk()
        ->assertSeeText(trans('dashboard::app.components.layouts.sidebar.sections'))
        ->assertSeeText(trans('dashboard::app.appearance.sections.index.create-btn'));
});
...
it('should delete the section', function () {
    ...
    deleteJson(route('admin.appearance.sections.delete', $section->id))
        ->assertOk()
        ->assertJsonPath('message', trans('dashboard::app.appearance.sections.delete-success'));
...
it('should place a duplicate directly below its original', function () {
    ...
    expect($order)->toBe([
        'First',
        'Second',
        'Second '.trans('dashboard::app.appearance.sections.index.copy-suffix'),
        'Third',
    ]);
});
    ...
    $walk = function (array $fields) use (&$walk, &$suspect) {
        foreach ($fields as $field) {
            ...
            foreach ($labels as $label) {
                if (
                    $label === ''
                    || $label === trans('dashboard::app.appearance.sections.edit.title')
